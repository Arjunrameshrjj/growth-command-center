...
import pytz
import concurrent.futures # Add this import at top if missing, or included in fetching logic
...
with tabs[3]: 
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        # Schedule the execution of all tasks
        future_m1_cust = executor.submit(get_kajabi_new_customers, date_m1_start, date_m1_end)
        future_m2_cust = executor.submit(get_kajabi_new_customers, date_m2_start, date_m2_end)
        future_m1_sales = executor.submit(get_kajabi_sales, date_m1_start, date_m1_end)
        future_m2_sales = executor.submit(get_kajabi_sales, date_m2_start, date_m2_end)
        future_products = executor.submit(get_kajabi_products)
        future_active = executor.submit(get_kajabi_active_customers, date_m1_start)
        future_offers = executor.submit(get_kajabi_offers)

        # Retrieve results as they complete (or wait for all)
        m1_count, m1_list, m1_total = future_m1_cust.result()
        m2_count, m2_list, m2_total = future_m2_cust.result()
        rev_m1, sales_m1 = future_m1_sales.result()
        rev_m2, sales_m2 = future_m2_sales.result()
        products = future_products.result()
        active_mtd, total_cust = future_active.result()
        offers_map = future_offers.result()
...
def get_renew_sheet_data(target_month_start, target_month_end):
    ...
    try:
        ...
        if date_col in df.columns:
            df['Parsed Date'] = pd.to_datetime(df[date_col], errors='coerce').dt.date
            mask = (df['Parsed Date'] >= target_month_start) & (df['Parsed Date'] <= target_month_end)
            df_filtered = df.loc[mask]
            
            total_rev = df_filtered[fee_col].sum()
